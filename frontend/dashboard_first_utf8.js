...
/* ΓöÇΓöÇ Animated Score Number ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇ Overview Dashboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
    ...
    <div className="p-6 md:p-10 animate-fade-in flex flex-col h-full bg-background" style={{ backgroundImage: "linear-gradient(to right, hsl(0,0%,10%) 1px, transparent 1px), linear-gradient(to bottom, hsl(0,0%,10%) 1px, transparent 1px)", backgroundSize: "40px 40px" }}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
        ...
        >
          ...
          <div className="mt-auto pt-4 w-full text-right text-sm font-bold text-blue-600 hover:underline">Open ΓåÆ</div>
        </div>
        ...
        >
          ...
          <div className="mt-auto pt-4 w-full text-right text-sm font-bold text-orange-600 hover:underline">Open ΓåÆ</div>
        </div>
        ...
        >
          ...
          <div className="mt-auto pt-4 w-full text-right text-sm font-bold text-green-600 hover:underline">Open ΓåÆ</div>
        </div>
        ...
        >
          ...
          <div className="mt-auto pt-4 w-full text-right text-sm font-bold text-purple-600 hover:underline">Open ΓåÆ</div>
        </div>
      </div>
                    ...
                    {Object.keys(llmMetrics.by_task || {}).length === 0 ? (
                      ...
                      Object.entries(llmMetrics.by_task).map(([task, data]) => (
                        <div key={task} className="text-sm font-bold flex justify-between gap-4 border-b border-gray-200 pb-2 last:border-0">
                          ...
                          <span className="shrink-0 text-gray-600">{data.success_rate_pct}% ΓÇó {data.avg_latency_ms}ms</span>
                        </div>
                      ))
                    )}
                  </div>
...
/* ΓöÇΓöÇ Tab Components (Shells for now) ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
            ...
            >
              {history.map((item, idx) => (
                <option key={item.id} value={idx}>
                  Iteration {history.length - idx} ΓÇó Score: {item.score_breakdown?.total_score || item.ats_score || 0} ΓÇó {new Date(item.created_at).toLocaleDateString()}
                </option>
              ))}
            </select>
          </div>
...
function JobTracker() {
  ...
  return (
    <div className="p-6 md:p-10 h-full flex flex-col animate-fade-in relative overflow-hidden bg-background" style={{ backgroundImage: "linear-gradient(to right, hsl(0,0%,10%) 1px, transparent 1px), linear-gradient(to bottom, hsl(0,0%,10%) 1px, transparent 1px)", backgroundSize: "40px 40px" }}>
      ...
      {/* ΓöÇΓöÇ Discover View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {activeView === "discover" && (
        <div className="flex-1 overflow-y-auto pr-2">
          ...
          {jobsLoading ? (
            ...
            <div className="text-center font-bold text-gray-500 py-12 bg-white border-[3px] border-black shadow-hard">No jobs found ΓÇö try a different role</div>
          ) : (
              ...
              {jobs.map((job) => (
                <div key={job.id} className="bg-white p-6 border-[3px] border-black shadow-hard flex flex-col gap-4 hover:-translate-y-1 hover:-translate-x-1 transition-transform">
                  <div>
                    ...
                    {job.salary && <p className="text-xs text-green-600 font-black mt-2 bg-green-50 inline-block px-2 py-1 border-2 border-green-200">≡ƒÆ░ {job.salary}</p>}
                  </div>
                  ...
                  <div className="flex gap-2 mt-auto pt-4 border-t-[2px] border-dashed border-gray-300">
                    ...
                    {job.url && <a href={job.url} target="_blank" rel="noopener noreferrer" className="text-xs font-black uppercase py-2 px-3 bg-white text-black border-[2px] border-black hover:bg-gray-100 transition-colors">Γåù</a>}
                  </div>
                </div>
              ))}
            </div>
      ...
      {/* ΓöÇΓöÇ Kanban View ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {activeView === "kanban" && (
                ...
                <div className="flex-1 space-y-4 overflow-y-auto min-h-40 p-2">
                  {colApps.length === 0 ? (
                    ...
                    <div key={app.id} className="bg-white border-[3px] border-black p-4 shadow-hard hover:-translate-y-1 hover:-translate-x-1 transition-transform relative group">
                      <button onClick={() => handleDelete(app.id)} className="absolute top-2 right-2 text-red-600 bg-red-100 border-[2px] border-red-600 w-6 h-6 flex items-center justify-center font-black opacity-0 group-hover:opacity-100 transition-opacity text-xs">Γ£ò</button>
      ...
      {/* ΓöÇΓöÇ AI Resume Match Modal ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */}
      {matchModal && (
        <div className="absolute inset-0 bg-background/95 backdrop-blur-sm z-20 flex items-center justify-center p-6 animate-fade-in">
          <div className="bg-card border-2 border-primary w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6">
            <div className="flex justify-between items-start mb-4">
              ...
              <button onClick={() => { setMatchModal(null); setMatchResult(null); }} className="font-mono text-muted-foreground hover:text-foreground text-sm">[ Γ£ò Close ]</button>
            </div>
...
const CATEGORIES = [
  ...
  },
  {
    ...
    resources: [
      ...
      { name: "System Design Primer", desc: "GitHub's most starred system design guide", url: "https://github.com/donnemartin/system-design-primer", tag: "GitHub, 250k Γ¡É" },
  ...
  },
  {
    ...
    resources: [
      { name: "Fraz SDE Sheet", desc: "By Mohammad Fraz (ex-Microsoft) ΓÇö 450 problems", url: "https://docs.google.com/spreadsheets/d/1-wKcV99KtO91dXdPkwmXGTdtyxAfk1mbPXQg81R9sFo", tag: "Microsoft cracker" },
      { name: "Siddharth Singh Google", desc: "By a Google SWE ΓÇö problem list + tips", url: "https://docs.google.com/spreadsheets/d/11tevcTIBQsIvRKIZLbSzCeN4mCO6wD4O5meyrAIfSXw", tag: "Google SWE" },
      { name: "DSA Cracker Sheet", desc: "By Aman Dhattarwal ΓÇö exam + placement focused", url: "https://docs.google.com/spreadsheets/d/1AVoprCGMBCpXD9DrR1HZj8Y_bGCKMa8VN5rmBcBSMT8", tag: "Placement" },
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 flex-1 overflow-y-auto">
          {cat.resources.map((r, i) => (
            <a key={i} href={r.url} target="_blank" rel="noopener noreferrer"
              ...
              <p className="text-xs font-mono font-bold group-hover:underline" style={{ color: cat.color }}>Open Γåù</p>
            </a>
          ))}
          ...
          {activeCategory === "dsa" && (
            <div onClick={() => setShowHolisticTracker(true)}
              ...
              <p className="text-xs font-mono font-bold text-primary group-hover:underline">Open Tracker ΓåÆ</p>
            </div>
          )}
          ...
          <div onClick={() => { setShowForm(true); setShowRoadmap(false); }}
            ...
            <p className="text-xs font-mono font-bold text-primary group-hover:underline">
              {roadmap ? "View / Regenerate ΓåÆ" : "Generate My Plan ΓåÆ"}
            </p>
          </div>
        </div>
      )}
      ...
      {showHolisticTracker && (
        <div className="absolute inset-0 bg-background/96 backdrop-blur-sm z-10 p-6 animate-fade-in overflow-y-auto">
          <div className="max-w-4xl mx-auto flex justify-end mb-4">
            <button onClick={() => setShowHolisticTracker(false)} className="text-muted-foreground hover:text-foreground font-mono text-sm border-2 border-border px-4 py-2">[ Γ£ò Close Tracker ]</button>
          </div>
        ...
        <div className="absolute inset-0 bg-background/96 backdrop-blur-sm z-10 flex items-center justify-center animate-fade-in p-6">
          <div className="brutalist-card bg-card border-2 border-primary p-8 w-full max-w-lg">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={() => setShowForm(false)} className="font-mono text-sm text-muted-foreground">[ Γ£ò ]</button>
            </div>
...
/* ΓöÇΓöÇ Main Dashboard ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  return (
    <>
      <div className="flex h-screen bg-background overflow-hidden font-sans">
        ...
        {/* Main content ΓÇö add left padding on mobile for hamburger button */}
