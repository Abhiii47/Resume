...
/* ΓöÇΓöÇ Animated Score Number ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
...
/* ΓöÇΓöÇ Overview Tab ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function OverviewTab({ history, setActiveTab, navigate, llmMetrics, llmMetricsLoading, showLlmMetrics = false }) {
  ...
  const statCards = [
    { label: 'Top ATS Score', value: latestScore, suffix: '/ 100', bg: 'hsl(var(--accent-500))', shadow: '6px 6px 0 #b34500', color: 'var(--foreground)' },
  ...
  const actionCards = [
    ...
    { label: 'Resume Lab', desc: 'Analyze & Fix Flaws', icon: UploadCloud, accent: 'hsl(var(--accent-500))', onClick: () => setActiveTab('workspace') },
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="mb-10">
        <p className="text-xs font-black uppercase tracking-widest mb-2" style={{ color: 'hsl(var(--accent-500))' }}>Agentic Career Hub</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mb-10">
        {actionCards.map((card, i) => (
          <div
            ...
            className="brutal-card p-5 flex flex-col gap-4 cursor-pointer transition-all hover:-translate-y-1 group"
          >
            <div
            ...
            <div className="text-right text-xs font-black" style={{ color: card.accent }}>Open ΓåÆ</div>
          </div>
        ))}
      </div>
      ...
      <div className="p-6" className="brutal-card">
        {llmMetricsLoading ? (
          <p className="text-sm font-bold animate-pulse" style={{ color: 'hsl(var(--accent-500))' }}>Loading metrics...</p>
        ) : !llmMetrics ? (
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5 mb-6">
              {[
                ...
                { label: 'Avg Latency', value: `${llmMetrics.avg_latency_ms || 0}ms`, color: 'hsl(var(--accent-500))' },
              ].map((m, i) => (
                <div key={i} className="p-4" className="brutal-card">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
              {[{ title: 'By Task', data: llmMetrics.by_task }, { title: 'By Provider', data: llmMetrics.by_provider }].map((section, i) => (
                <div key={i} className="p-4" className="brutal-card">
                  <p className="text-xs font-black uppercase tracking-widest mb-4 pb-2" style={{ color: 'var(--muted-foreground)', borderBottom: '1px solid #000' }}>{section.title}</p>
...
/* ΓöÇΓöÇ Resume Workspace ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  const scoreItems = selectedAnalysis ? [
    ...
    },
    {
      ...
      value: selectedAnalysis.score_breakdown?.impact_metrics
          ...
          || 0,
      max: 35, color: 'hsl(var(--accent-500))'
    },
  ] : [];
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full" >
      ...
      <div
        ...
        style={{ borderRight: '1px solid #000', background: '#fdfbf7' }}
      >
        ...
        >
          <form onSubmit={handleUpload} className="flex flex-col sm:flex-row gap-3">
            ...
            >
              ...
              <UploadCloud className="w-5 h-5 shrink-0" style={{ color: 'hsl(var(--accent-500))' }} />
        ...
        {history && history.length > 0 && (
          <div
            ...
            className="brutal-card"
          >
            <span className="text-xs font-black uppercase tracking-widest" style={{ color: 'hsl(var(--accent-500))' }}>Evolution Timeline</span>
            <select
              value={selectedIndex}
              onChange={e => setSelectedIndex(Number(e.target.value))}
              className="text-xs font-bold p-1 outline-none"
              style={{ background: '#fff', border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--muted-foreground)', cursor: 'pointer' }}
            >
        ...
        <div
          ...
          style={{ minHeight: 200, border: '1px solid #000', boxShadow: '4px 4px 0 #000', background: '#fdfbf7' }}
        >
          {pdfUrl ? (
            <object data={pdfUrl} type="application/pdf" className="w-full h-full" style={{ minHeight: 300 }}>
              <div className="h-full flex flex-col items-center justify-center gap-2 p-4" style={{ color: 'var(--muted-foreground)' }}>
                ...
                <a href="#" onClick={(e) => { e.preventDefault(); const a = document.createElement("a"); a.href = pdfUrl; a.download = "resume.pdf"; document.body.appendChild(a); a.click(); document.body.removeChild(a); }} className="text-xs font-black" style={{ color: "hsl(var(--accent-500))" }}>Download PDF</a>
              </div>
            </object>
          ) : selectedAnalysis ? (
      ...
      >
        <h2 className="text-3xl font-black uppercase mb-6" style={{ letterSpacing: '-0.02em', color: 'hsl(var(--accent-500))' }}>Diagnostics & Fixes</h2>

        {!selectedAnalysis ? (
          <div
            className="p-10 text-center font-bold"
            className="brutal-card text-muted-foreground"
          >
          ...
          <div className="space-y-6">
            ...
            {/* Flaws & Suggestions */}
            <div className="relative overflow-hidden" className="brutal-card">
                ...
                {selectedAnalysis.gemini_suggestions && selectedAnalysis.gemini_suggestions.length > 0 ? (
                  <div className="space-y-4">
                    {selectedAnalysis.gemini_suggestions.map((s, i) => (
                      <div key={i} className="p-4 text-sm" style={{ background: '#fdfbf7', border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--foreground)' }}>
                        ...
                        {rewrittenBullets[i] ? (
                          <div className="mt-3 p-3 text-sm" style={{ background: 'rgba(0,119,255,0.06)', borderLeft: '3px solid hsl(24,100%,50%)', color: 'var(--muted-foreground)' }}>
                            <span className="text-xs font-black uppercase block mb-1" style={{ color: 'hsl(var(--accent-500))' }}>AI Rewrite:</span>
                            {rewrittenBullets[i]}
                          </div>
                        ) : (
                          <button
                            ...
                            style={{ background: rewritingIndex === i ? '#222' : 'hsl(var(--accent-500))', color: rewritingIndex === i ? '#555' : '#111', border: '1px solid #000', boxShadow: '4px 4px 0 #000', cursor: rewritingIndex === i ? 'not-allowed' : 'pointer' }}
                          >
                            {rewritingIndex === i ? 'Generating...' : 'Rewrite Bullet ΓåÆ'}
                          </button>
                        )}
                      </div>
                    ))}
...
/* ΓöÇΓöÇ Job Tracker ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
function JobTracker() {
  ...
  const STAGE_COLORS = { wishlist: '#6b7280', applied: 'hsl(var(--accent-500))', interview: '#3b82f6', offer: '#22c55e', rejected: '#ef4444' };
  ...
  const inputStyle = { background: '#fff', border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--foreground)', padding: '10px 14px', fontFamily: 'inherit', fontSize: 13, outline: 'none', width: '100%' };
    ...
    >
        ...
        <div className="flex gap-3">
          {['discover', 'kanban'].map(view => (
            <button
              ...
              style={{
                background: activeView === view ? 'hsl(var(--accent-500))' : 'transparent',
                color: activeView === view ? '#111' : '#666',
                border: `2px solid ${activeView === view ? 'hsl(var(--accent-500))' : '#333'}`,
              }}
            >
          ...
          <button
            ...
            style={{ background: showAdd ? '#333' : '#fff', color: 'var(--foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}
          >
            {showAdd ? 'Γ£ò Cancel' : '+ Add Job'}
          </button>
        </div>
      </div>
      ...
      {showAdd && (
        ...
        >
          ...
          <button type="submit" className="px-6 py-2.5 font-black uppercase text-sm" style={{ background: 'hsl(var(--accent-500))', color: 'var(--foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000', height: 42 }}>Save</button>
        </form>
      )}
      ...
      {activeView === 'discover' && (
        <div className="flex-1 overflow-y-auto">
          <div className="flex gap-3 mb-5">
            ...
            />
            <button onClick={() => fetchJobs(jobSearch)} className="px-6 font-black uppercase text-sm" style={{ background: 'hsl(var(--accent-500))', color: 'var(--foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>Search</button>
          </div>

          {jobsLoading ? (
            <div className="text-center font-bold animate-pulse py-12" style={{ color: 'hsl(var(--accent-500))', background: '#fdfbf7', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>Fetching live jobs...</div>
          ) : jobs.length === 0 ? (
            <div className="text-center font-bold py-12" style={{ color: '#444', background: '#fdfbf7', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>No jobs found ΓÇö try a different role</div>
          ) : (
                ...
                >
                  <div>
                    <div className="flex justify-between items-start gap-2">
                      ...
                      {job.posted && <span className="text-[10px] font-bold shrink-0 px-2 py-0.5" style={{ border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--muted-foreground)' }}>{job.posted}</span>}
                    </div>
                    ...
                    {job.salary && <p className="text-xs font-black mt-2 inline-block px-2 py-1" style={{ background: 'rgba(34,197,94,0.08)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.2)' }}>≡ƒÆ░ {job.salary}</p>}
                  </div>
                  {job.tags && job.tags.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {job.tags.map((t, i) => <span key={i} className="text-[10px] font-bold px-2 py-0.5" style={{ background: 'var(--secondary)', color: '#888', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>{t}</span>)}
                    </div>
                  )}
                  <div className="flex gap-2 mt-auto pt-3" style={{ borderTop: '1px dashed #1f1f1f' }}>
                    <button onClick={() => handleMatchResume(job)} className="flex-1 text-xs font-black uppercase py-2 flex items-center justify-center gap-1 transition-colors" style={{ background: 'hsl(var(--accent-500))', color: 'var(--foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>
                    ...
                    {job.url && <a href={job.url} target="_blank" rel="noopener noreferrer" className="text-xs font-black uppercase py-2 px-3 transition-colors" style={{ background: 'transparent', color: 'var(--muted-foreground)', border: '2px solid #333' }}>Γåù</a>}
                  </div>
                </div>
              ))}
            </div>
          )}
          {/* Career Comms Lab ΓÇö available in Learning Hub */}
          <div
          ...
          >
            ...
            <p className="text-xs font-black" style={{ color: '#3b82f6' }}>Available in Learning Hub ΓåÆ</p>
          </div>
        </div>
      )}
            ...
            return (
              <div key={status} className="w-72 shrink-0 flex flex-col">
                <div
                  ...
                  style={{ background: '#fdfbf7', border: '1px solid #000', boxShadow: '4px 4px 0 #000', borderBottomColor: STAGE_COLORS[status], borderBottomWidth: 4 }}
                >
                  <span style={{ color: STAGE_COLORS[status] }}>{status}</span>
                  <span className="px-2 text-xs" style={{ background: 'var(--secondary)', color: '#888', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>{colApps.length}</span>
                </div>
                  ...
                  {colApps.length === 0 ? (
                    ...
                    >
                      ...
                      <select
                        ...
                        style={{ background: '#fff', border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--foreground)', padding: '6px 8px', cursor: 'pointer' }}
                      >
      ...
      {matchModal && (
        ...
        >
          <div className="w-full max-w-2xl max-h-[85vh] overflow-y-auto p-6" style={{ background: '#fdfbf7', border: '2px solid #222', boxShadow: '8px 8px 0 hsl(24,100%,50%)' }}>
            <div className="flex justify-between items-start mb-5" style={{ borderBottom: '1px solid #000', paddingBottom: '1rem' }}>
              <div>
              ...
              <button onClick={() => { setMatchModal(null); setMatchResult(null); }} className="text-xs font-black uppercase px-3 py-1.5" style={{ color: 'var(--muted-foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>Γ£ò Close</button>
            </div>

            {matchLoading && <div className="text-center py-10 font-bold animate-pulse" style={{ color: 'hsl(var(--accent-500))' }}>Analyzing resume fit...</div>}
              ...
              <div className="space-y-5">
                ...
                >
                  <div
                    ...
                    style={{ color: matchResult.match_score >= 70 ? '#22c55e' : matchResult.match_score >= 50 ? 'hsl(var(--accent-500))' : '#ef4444', fontFamily: 'Playfair Display, serif' }}
                  >
                ...
                {matchResult.resume_tweaks?.length > 0 && (
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-widest mb-2" style={{ color: 'hsl(var(--accent-500))' }}>Resume Fixes</p>
                    <div className="space-y-2">
                      {matchResult.resume_tweaks.map((t, i) => (
                        <div key={i} className="p-3 text-sm" style={{ background: '#fdfbf7', border: '1px solid #000', boxShadow: '4px 4px 0 #000', color: 'var(--foreground)' }}>
                          <span className="text-xs font-black uppercase" style={{ color: 'hsl(var(--accent-500))' }}>{t.section}: </span>{t.action}
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <ul className="space-y-1.5">
                      {matchResult.strengths.map((s, i) => (
                        <li key={i} className="flex items-start gap-2 text-sm" style={{ color: '#888' }}>
                          <span style={{ color: '#22c55e', marginTop: 2 }}>Γ£ô</span> {s}
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                <button onClick={() => saveJobToTracker(matchModal)} className="w-full py-3 font-black uppercase text-sm flex items-center justify-center gap-2" style={{ background: 'hsl(var(--accent-500))', color: 'var(--foreground)', border: '1px solid #000', boxShadow: '4px 4px 0 #000' }}>
...
/* ΓöÇΓöÇ Resource Hub ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ */
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex" >
        <div className="hidden lg:flex" style={{ width: 240, minWidth: 240, background: '#fdfbf7', borderRight: '1px solid #000' }} />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <div className="text-sm font-black uppercase tracking-widest mb-3 animate-pulse" style={{ color: 'hsl(var(--accent-500))' }}>Loading your dashboard...</div>
            <div className="flex gap-1.5 justify-center">
              {[1, 2, 3].map(i => <div key={i} className="w-2 h-2 rounded-full animate-bounce" style={{ background: 'hsl(var(--accent-500))', animationDelay: `${i * 0.15}s` }} />)}
            </div>
          </div>
        </div>
      </div>
