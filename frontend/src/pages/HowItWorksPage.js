...
export default function HowItWorksPage() {
  ...
  const steps = [
    { num: '01', title: 'Upload Your Resume', desc: 'Drop any PDF resume. Our parser extracts all text, skills, sections, and formatting signals in milliseconds using pdfminer.', color: 'hsl(24,100%,50%)' },
    ...
    <div className="min-h-screen" style={{ background: 'hsl(40,30%,92%)' }}>
      ...
      <div className="max-w-4xl mx-auto px-6 py-20">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs font-black uppercase tracking-widest" style={{ color: 'hsl(24,100%,50%)' }}>How It Works</span>
        </div>
        <h1 className="font-display-serif text-5xl md:text-6xl mb-6" style={{ color: '#111', letterSpacing: '-0.03em' }}>
          5 Steps to a<br /><span className="inline-block px-2" style={{ background: 'hsl(24,100%,50%)', color: '#111', border: '2px solid #000' }}>Perfect Resume</span>
        </h1>
        ...
        <div className="space-y-5 mb-20">
          {steps.map((step, i) => (
            <div key={i} className="flex gap-6 p-6 shadow-hard transition-transform hover:-translate-y-0.5" style={CARD}>
              <div className="flex flex-col items-center shrink-0">
                <div className="w-12 h-12 flex items-center justify-center font-black text-lg shadow-hard-sm" style={{ background: step.color, color: step.color === 'hsl(24,100%,50%)' || step.color === '#f59e0b' ? '#111' : '#fff', border: '2px solid #000' }}>
                  {step.num}
                </div>
                {i < steps.length - 1 && <div className="w-0.5 flex-1 mt-2" style={{ background: '#ccc' }} />}
              </div>
              <div className="flex-1">
                <h3 className="font-black text-lg uppercase mb-1" style={{ color: '#111', letterSpacing: '-0.02em' }}>{step.title}</h3>
                <p className="text-sm" style={{ color: '#666', lineHeight: 1.7 }}>{step.desc}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-3 mb-6">
          <span style={{ width: 28, height: 2, background: 'hsl(24,100%,50%)', display: 'inline-block' }} />
          <span className="text-xs font-black uppercase tracking-widest" style={{ color: 'hsl(24,100%,50%)' }}>Frequently Asked</span>
        </div>
        <div className="space-y-4 mb-16">
          {faqs.map((faq, i) => (
            <div key={i} className="p-5 shadow-hard" style={CARD}>
              <p className="font-black text-sm uppercase mb-2" style={{ color: '#111' }}>{faq.q}</p>
              <p className="text-sm" style={{ color: '#666', lineHeight: 1.6 }}>{faq.a}</p>
            </div>
          ))}
        </div>
