    ...
    <div className="min-h-screen" style={{ background: 'hsl(40,30%,92%)' }}>
      ...
      <div className="max-w-5xl mx-auto px-6 py-20">
        <div className="flex items-center gap-3 mb-4">
          ...
          <span className="text-xs font-black uppercase tracking-widest" style={{ color: 'hsl(24,100%,50%)' }}>Resume Templates</span>
        </div>
        <h1 className="font-display-serif text-5xl mb-4" style={{ color: '#111', letterSpacing: '-0.03em' }}>
          ATS-Optimized <span className="inline-block px-2" style={{ background: 'hsl(24,100%,50%)', color: '#111', border: '2px solid #000' }}>Templates</span>
        </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
          {TEMPLATES.map((t) => (
            <div key={t.id} className="flex flex-col transition-transform hover:-translate-y-1 shadow-hard" style={CARD}>
              {/* Template preview area */}
              <div className="p-8 flex items-center justify-center" style={{ background: 'hsl(40,28%,88%)', borderBottom: '2px solid #000', minHeight: 180 }}>
              ...
              <div className="p-6 flex-1 flex flex-col">
                <h3 className="font-black text-lg uppercase mb-2" style={{ color: '#111' }}>{t.name}</h3>
                <p className="text-sm mb-4" style={{ color: '#666', lineHeight: 1.6 }}>{t.desc}</p>
                ...
                <p className="text-[10px] font-black uppercase tracking-widest mb-3" style={{ color: '#aaa' }}>Sections</p>
                ...
                <button
                  ...
                  className="mt-auto w-full py-3 font-black uppercase text-sm text-center shadow-hard-sm transition-transform hover:-translate-y-0.5"
                  style={{ background: t.color, color: t.color === 'hsl(24,100%,50%)' || t.color === '#f59e0b' ? '#111' : '#fff', border: '2px solid #000' }}
                >
                  Use This Template →
                </button>
              </div>
